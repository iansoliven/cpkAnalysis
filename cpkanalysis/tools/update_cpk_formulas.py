...
def _require_column(header_map: dict[str, int], header: str) -> int:
    key = sheet_utils.normalize_header(header)
    column = header_map.get(key)
    if column is None:
        raise ValueError(f"Template sheet missing required column '{header}'.")
    return column


def _ensure_column(ws, header_row: int, header_map: dict[str, int], header: str) -> int:
    key = sheet_utils.normalize_header(header)
    column = header_map.get(key)
...
def _cell(letter: str, row: int) -> str:
    return f"${letter}{row}"


def _sheet_cell(sheet_ref: str, letter: str, row: int) -> str:
    return f"{sheet_ref}${letter}{row}"
...
def update_workbook(path: Path, template_sheet: str) -> None:
    ...
    ll_spec_col = _require_column(header_map, "LL_SPEC")
    ul_spec_col = _require_column(header_map, "UL_SPEC")
    ll_ate_col = _require_column(header_map, "LL_ATE")
    ul_ate_col = _require_column(header_map, "UL_ATE")
    cpl_col = _require_column(header_map, "CPL")
    cpu_col = _require_column(header_map, "CPU")
    cpk_col = _require_column(header_map, "CPK")
    ll_2cpk_col = _require_column(header_map, "LL_2CPK")
    ul_2cpk_col = _require_column(header_map, "UL_2CPK")
    cpk_2_col = _require_column(header_map, "CPK_2.0")
    ll_3iqr_col = _require_column(header_map, "LL_3IQR")
    ul_3iqr_col = _require_column(header_map, "UL_3IQR")
    cpk_3iqr_col = _require_column(header_map, "CPK_3IQR")
    ll_prop_col = _require_column(header_map, "LL_PROP")
    ul_prop_col = _require_column(header_map, "UL_PROP")
    cpk_prop_col = _require_column(header_map, "CPK_PROP")
