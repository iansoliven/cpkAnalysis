...
def choose_bins(values: List[float], low: float, high: float) -> np.ndarray:
    if not values:
        return np.linspace(low, high, 2)
    span = high - low
    if span <= 0:
        span = max(abs(low) * 0.1, 1.0)
        high = low + span
...
def build_chart_image(test: TestData) -> Optional[BytesIO]:
    ...
    axis_low, axis_high = determine_axis_limits(test.measurements, low_declared, high_declared)
    bin_edges = choose_bins(test.measurements, axis_low, axis_high)
...
def add_charts_to_workbook(workbook_path: Path, output_path: Path, tests: Dict[Tuple[str, Optional[str]], TestData], charts_per_row: int) -> None:
    ...
    charts_ws = wb.create_sheet(CHARTS_SHEET_NAME)
    charts_ws["A1"] = "Charts generated by addCharts.py"
