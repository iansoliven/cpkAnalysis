#!/usr/bin/env python3
"""Generate boxplot charts grouped by Test Name and Lot.
...
builds boxplots of the Measurement column, overlays them by INT/Interval subgroups,
and annotates the mean shift between the smallest and largest INT buckets. Separate
sheets named `Boxplot_<EVENT>` are inserted immediately after the Measurements
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Add boxplot charts for each Test Name/Lot pairing in the combined workbook."
    )
    parser.add_argument(
        "workbook",
        nargs="?",
...
def _boxplot_sheet_name(event_name: str, existing_names: Iterable[str]) -> str:
    label = event_name or "Unknown Event"
    base_title = f"{BOXPLOT_SHEET_PREFIX}{label}"
...
def build_boxplot_image(test: TestData, lot_data: LotData) -> Optional[BytesIO]:
    ...
    padding = max(abs(axis_high_raw - axis_low_raw) * 0.05, 1e-6)
    ...
    if low_declared is not None:
        ax.axhline(low_declared, color="#C0504D", linestyle="--", linewidth=2, label="Low Limit")
    if high_declared is not None:
        ax.axhline(high_declared, color="#9BBB59", linestyle="--", linewidth=2, label="High Limit")

    legend_handles = [Patch(facecolor=color, edgecolor="#1F497D", alpha=0.7, label=label) for color, label in zip(colors, group_labels)]
    handles, labels = ax.get_legend_handles_labels()
    if legend_handles or handles:
        ax.legend(legend_handles + handles, [h.get_label() for h in legend_handles] + labels, loc="upper left", bbox_to_anchor=(0.02, 0.98))
...
def add_boxplots_to_workbook(
    ...
    for sheet_name in list(wb.sheetnames):
        if sheet_name.startswith(BOXPLOT_SHEET_PREFIX) or sheet_name == SHIFT_SUMMARY_SHEET_NAME:
            del wb[sheet_name]
    ...
    for event_name, event_data in events.items():
        sheet_name = _boxplot_sheet_name(event_name, wb.sheetnames)
        charts_ws = wb.create_sheet(sheet_name, index=insert_index)
        insert_index = wb.sheetnames.index(sheet_name) + 1
        charts_ws["A1"] = (
            f"Boxplot charts generated by {Path(__file__).name} for event '{event_name}' ({sheet_name})"
        )
        ...
        for test_idx, test in enumerate(sorted_tests):
            ...
            for col_idx, lot_name in enumerate(lot_names):
                ...
                image_stream = build_boxplot_image(test, lot_data)
...
def main() -> None:
    ...
    summary_count = add_boxplots_to_workbook(workbook_path, output_path, events, max_lots=max(args.max_lots, 0))
    total_boxplots = sum(len(test.lots) for event in events.values() for test in event.tests.values())
    total_tests = sum(len(event.tests) for event in events.values())
    print(
        f"Added boxplot sheets for {len(events)} event(s) with {total_boxplots} boxplot(s) across "
